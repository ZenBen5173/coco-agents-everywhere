...
const StreakCalendar = React.forwardRef<HTMLDivElement, StreakCalendarProps>(
  (
    ...
    // Local change from the registry version, for two reasons.
    //
    // The original pairs `--info` with `--info-foreground`, assuming the second
    // means "a colour that reads on the first". In this project it means "a
    // darker shade of info", so the pair came out at 1.34:1 — purple on purple,
    // with the day number invisible.
    //
    // The tint also says the right thing. A frozen day is not an earned day:
    // the run survived it, but nothing happened. A solid chip shouted as loudly
    // as a day that was actually kept, so the fill is dropped to a wash and the
    // colour moved into the number.
    const freezeColorStyles = {
      "--freeze-color": "color-mix(in oklab, var(--info) 20%, transparent)",
      // Pulled a fifth of the way towards the page's text colour: 4.70:1 on
      // the wash, where bare `--info` managed 3.41 and small text needs 4.5.
      // Mixing towards `--foreground` rather than white keeps it correct when
      // the theme flips and the text colour goes dark.
      "--freeze-foreground-color":
        "color-mix(in oklab, var(--info) 80%, var(--foreground))",
